...
def rule0(simulation, indices):  # A, 1: safe distance to ped
    return pairwise_distance_margin(simulation, indices, other_actor_idx=5, margin=8, reducer=np.min)
...
def rule4(simulation, indices):  # B, 4: safe distance to adv4
    return pairwise_distance_margin(simulation, indices, other_actor_idx=4, margin=8, reducer=np.min)
