...
def test_dce_requires_rulebook():
    params = DotMap(alpha=0.8, thres=0.0, rulebook=None)
    params.cont.buckets = 3
    params.cont.dist = None

    with pytest.raises(ValueError, match="dce_params.rulebook must be set"):
        DynamicRulebookCrossEntropySampler(Box((0, 1)), params)


def test_dce_rejects_nonconsecutive_priority_graph_ids():
    ...
    with pytest.raises(
        ...
        DynamicRulebookCrossEntropySampler(Box((0, 1)), _make_params(rb))


def test_dce_round_robin_and_fixed_sampler_routing():
    rb = RulebookStub(
        priority_graphs={0: _make_graph(), 1: _make_graph()}, using_sampler=-1
    )
    sampler = DynamicRulebookCrossEntropySampler(Box((0, 1)), _make_params(rb))
    ...
    fixed = DynamicRulebookCrossEntropySampler(Box((0, 1)), _make_params(rb_fixed))
